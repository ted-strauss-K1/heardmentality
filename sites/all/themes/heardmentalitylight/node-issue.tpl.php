      ...
      <?php if ($page) : ?>
        <?php if ($node->moderate == MODERATION_ALLOWED) : ?>
          ...
          <?php if (module_exists('moderation') && moderation_check_perm()) : ?>
            <?php print l(__('issue-page-moderation', 'moderation'), 'moderation/issue/' . $node->nid, array(
              'attributes' => array(
                'class'          => 'moderation',
                'original-title' => __('issue-page-moderation', 'moderation'),
              )
            )); ?>
          <?php endif; ?>
        ...
        <?php elseif (!empty($node->field_base_issue[0]['nid'])) : ?>
          ...
          <?php if ($node->field_base_issue[0]['safe']['status']) : ?>
            <?php print __('issue-page-duplicates', 'Duplicates issue') ?> <?php
            $base_issue = node_load($node->field_base_issue[0]['nid']);
            print l(issue_translate($base_issue, 'title'), 'node/' . $base_issue->nid);
            ?>
          <?php else : ?>
            <?php print __('issue-page-duplicates-closed', 'Closed as the duplicate to other content on site'); ?>
          <?php endif; ?>
...
<?php if ($page) : ?>
  ...
  <div class="ul_wrapper">
    <h2 class="din"><?php print __('issue-page-debate', 'Debate'); ?></h2>
  </div>
  <ul class="tabs-content" style="margin: 22px 0 0;">
    <li class="active" id="simpleTab">
          ...
          <?php if (ranks_permission(0, $node->uid)) : // todo has sufficient rank ?>
            ...
            <div class="expanding arg">
              <h6 value="Reply" id="add-arg"
                  class="button <?php print theme('user_login_modal_class'); ?>"><?php print __('issue-page-debate-add', 'Add'); ?></h6>
            </div>
          ...
          <div id="leave_comment_area" class="leave-a-comment hidden_ar" style="display: none">
            ...
            <?php else : ?>
              <div class="<?php print theme('user_login_modal_class', TRUE) ?>" align="center"><b>
                  <?php print __('issue-page-debate-signin', 'Please log in to participate in the debate') ?>
                </b></div>
            <?php endif; ?>
          </div>

          <div id="analytics-area" class="hidden_deb" style="display: none;">
            <h2><span><?php print __('issue-page-debate-stats', 'Debate statistics'); ?></span></h2>
      ...
      <div id="debate_list_area">
        <h2>
          <span
            class="argcount"><?php print intval($node->info['argument']); ?></span>&nbsp;<span><?php print __('issue-page-info-arguments', 'Arguments'); ?>
            &nbsp;&amp;&nbsp;</span> <span class="rescount"><?php print intval($node->info['resource']); ?></span>&nbsp;<span><?php print __('issue-page-info-references', 'References'); ?></span>

          <div class="show_only">
            <span class="button" id="show_filter">&#9660; <?php print __('issue-page-debate-filter-showonly', 'Show only'); ?></span>

            <div class="inc">
              <form id="inc_ref">
                <input type="checkbox" checked="yes" value="include references" id="inc_check"/> <label
                  for="inc_check"><?php print __('issue-page-debate-filter-references', 'Include References'); ?></label>
              </form>
            </div>
            <div class="popup hidden" id="filter_content">
              <span class="title"><?php print __('issue-page-debate-filter-showall', 'Show all'); ?></span>
              ...
              <?php endforeach; ?>
              <span class="title reset"><?php print __('issue-page-debate-filter-reset', 'Reset filters'); ?></span>
            </div>
          </div>
        </h2>
        ...
        <ul>
          <li><a
              href="<?php print url('argument/tab'); ?>/<?php print $node->nid; ?>/0/0"><?php print __('issue-page-debate-tab-recent', 'recent'); ?></a>
          </li>
          <li><a
              href="<?php print url('argument/tab'); ?>/<?php print $node->nid; ?>/1/0"><?php print __('issue-page-debate-tab-older', 'older'); ?></a>
          </li>
          <li><a
              href="<?php print url('argument/tab'); ?>/<?php print $node->nid; ?>/2/0"><?php print __('issue-page-debate-tab-supported', 'supported'); ?></a>
          </li>
        </ul>
        <div id="ui-tabs-1">
          <?php
