...
function heardmentalitylight_preprocess_page(&$vars) {
    ...
    if ($node = $vars['node']) {
      switch ($node->type) {
        case 'issue' :
          ...
          drupal_set_subtitle(__('Issue', array('@code' => 'issue-page-title')));
  ...
  # css files
  global $theme_path;
  drupal_add_css($theme_path . '/stylesheets/header.css');
  drupal_add_css($theme_path . '/stylesheets/layout.css');
  drupal_add_css($theme_path . '/stylesheets/footer.css');
  if (is_node_page()) {
    if ($node = $vars['node']) {
