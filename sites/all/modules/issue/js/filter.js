...
jQuery('#add-new-debate-form').live('submit', function(e){
  ...
  if(title.length < 2){
    ...
    jQuery('#deb-err').html('<span>You must choose atleast one suppose or oppose.</span>');
    return false;
  }else{
...
function createUploader(){
  var uploader = new qq.FileUploader({
    ...
    onSubmit: function(id, fileName){
      jQuery.growlUI('','File uploading please wait!');
    },
    onProgress: function(id, fileName, loaded, total){
      jQuery.growlUI('','File uploading please wait!');
    },
    onComplete: function(id, fileName, responseJSON){
      jQuery('#docpath').val(responseJSON.filename);
      jQuery.growlUI('','File uploaded successfully!');
    },
