<div class="dark-grey-box top clearfix">
  <div class="profile-meta profile_page_wrapper">
    <i class="icon cat text icon-groups-friends"></i> <label for="profile"
                                                             class="white_text with_icon"><?php print t('You\'re following'); ?></label>
  </div>
  ...
  <?php if (!empty($users)) : ?>
    <div class="following clearfix">
      <?php global $user; ?>
      <?php print l(t('See all') . ' (' . $users_count . ')', 'user/profile/follow/' . $user->uid . '/following', array('attributes' => array('class' => 'more'))); ?>
    </div>
  <?php endif; ?>
</div>
