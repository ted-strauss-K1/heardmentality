...
if (isset($_REQUEST['scid']) || isset($_REQUEST['cid'])) {
    ...
    if (!empty($qids)) {
        $numrancnt = db_result(db_query("SELECT COUNT(*) from  question   where cid=" . $cid . " and qid in (" . $qids . ")  "));
        ...
        while ($flwing_result = db_fetch_object($rs_folwin)) {
            ...
            $numrancnts = db_result(db_query("SELECT COUNT(*) from  question   where cid=" . $cid . " and scid=" . $flwing_result->cat_id . " and qid in (" . $qids . ")"));
